...
// Helper Aktive Zeile 
  ...
  useEffect(() => {
    ...
    if (handledFocusTokenRef.current === focusLineRequest.token) return;
    const textarea = textareaRef.current;
    if (!textarea) return;
    handledFocusTokenRef.current = focusLineRequest.token;

    const lines = textarea.value.split('\n');
    const targetLine = Math.max(0, Math.min(focusLineRequest.line, lines.length - 1));
    let cursorPos = 0;
    ...
    onActiveLineChange?.(targetLine);
    if (showLineNumbers && lineNumbersRef.current) {
      lineNumbersRef.current.scrollTop = targetScrollTop;
    }
  }, [focusLineRequest, lineMetrics.lineHeight, onActiveLineChange, showLineNumbers]);
...
const getCursorPosition = () => {
  ...
  const maxLeft = Math.max(8, container.clientWidth - toolbarWidth - 8);
  if (left < 8) left = 8;
  ...
  const calculateToolbarPosition = () => {
    ...
    const container = editorContainerRef.current;
    if (!textarea || !toolbar || !container) return;
    ...
    let left =
      (textareaRect.left - containerRect.left) +
      paddingLeft +
      ((startCharIndex + midCharOffset) * charWidth);
    ...
    if (top < 8) {
      ...
      top =
        (textareaRect.top - containerRect.top) +
        paddingTop +
        ((startLineIndex + 1) * lineHeight) -
        scrollTop +
        4;
    }

    // Horizontal bounds within editor container
    const maxLeft = Math.max(8, container.clientWidth - toolbarWidth - 8);
    if (left < 8) {
      left = 8;
  ...
  }, [value, showCommandMenu, lineMetrics.lineHeight, lineMetrics.paddingTop]);
...
// neu Auto election bei UnWRAP
  ...
  const plusMenuItems: ZenPlusMenuItem[] = [
    ...
    { id: 'table', label: 'Table', icon: faTable, description: 'Markdown-Tabelle einfügen', action: () => {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Tab' && !showCommandMenu) {
      ...
      const selectionBlock = current.slice(lineStart, lineEnd);
      const lines = selectionBlock.split('\n');

      if (start !== end || selectionBlock.includes('\n')) {
        if (e.shiftKey) {
          const outdented = lines
            ...
            .join('\n');
          const next = current.slice(0, lineStart) + outdented + current.slice(lineEnd);
          onChange(next);
          setTimeout(() => {
            textarea.focus();
      ...
      if (e.shiftKey) {
        ...
        if (before.endsWith('\t')) {
          ...
          const next = current.slice(0, start - 2) + current.slice(end);
          onChange(next);
          setTimeout(() => {
            textarea.focus();
    ...
    <div className="w-full relative">
      ...
      <div className={`flex gap-4 ${showPreview ? 'md:flex-row flex-col' : ''}`}>
        ...
        <div
          ...
          className={`${showPreview ? 'md:w-1/2 w-full' : 'w-full'} flex-1 min-w-0 relative`}
        >
          {/* Floating Toolbar - Appears on text selection */}
          {showToolbar && !showCommandMenu && (
            <div
              ref={toolbarRef}
              className="absolute z-50 flex items-center gap-0.5 p-0.5 
